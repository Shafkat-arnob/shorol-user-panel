...
const TabFilters = (props: any) => {
  ...
  ({ subCategoryList, colorList } = props);

  const { getAllList, list, getProductByFilters } = useProductStore(
    (state) => state
  );
  ...
  const applyFilter = () => {
    console.log(colorsState, categoriesState);
    getProductByFilters({
      colorName: colorsState.join(","),
      categoryName: categoriesState.join(","),
    });
  };
